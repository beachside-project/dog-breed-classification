...
まず、画像を表示するコントロールを追加します。メニューの **挿入** > **メディア** > 画像をクリックします。
...
以下の画像では、タイトルやラベルを追加したり、分析ボタンの Visible プロパティに「!IsEmpty(dogImage.Url)」と設定することで画像がないときにはボタンを非表示にするなどしてみました。
...
**おめでとうございます！**:star2:  
機械学習のコーディングなしで、動画をアップロードしたことをトリガーに、犬の画像分類ができるスマホアプリを開発することができました。
